...
static int open_input(const char* filename)
{
	...
	if(avformat_find_stream_info(inputFile.fmt_ctx, NULL) < 0) {
		printf("Failed to retreive input steram information\n");
		return -2;
	}
...
int main(int argc, char* argv[])
{
	...
	for(index = 0; index < inputFile.fmt_ctx->nb_streams; index++) {
		...
		// flush filter
		out_stream_index = (index == inputFile.v_index) ? outputFile.v_index : outputFile.a_index;
		ret = filter_encode_write_frame(NULL, out_stream_index);
		if(ret < 0) {
			printf("Error occurred while flushing filter context\n");
			break;
		}
